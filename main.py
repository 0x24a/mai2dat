...
def main(
    ...
    try:
        ...
        exit(1)
    print("[yellow](1)[/yellow] Checking FFMpeg... ", end="")
    if check_ffmpeg(ffmpeg_path):
        ...
        exit(1)
    print("[yellow](2)[/yellow] Checking FFProbe... ", end="")
    if check_ffprobe(ffprobe_path):
        ...
        exit(1)
    print("[yellow](3)[/yellow] Copying file to the temporary folder... ", end="")
    temp_folder = mkdtemp()
    shutil.copy(source, path.join(temp_folder, path.basename(source)))
    print("[green]OK[/green]")
    print("[yellow](4)[/yellow] Converting the source file to VP9 (IVF) encoding...")
    task = FFMpegTask(
        ffmpeg_path,
    ...
    with Live(Text(""), refresh_per_second=16) as live:
        ...
        with open(path.join(temp_folder, "target.usm"), "wb") as f:
            for chunk in usm.stream(OpMode.ENCRYPT, encoding="shift-jis"):
                live.update(f"[bold][yellow](5)[/yellow][/bold] Converting to USM (.dat)... Chunk {chunk_n}")
                chunk_n += 1
                f.write(chunk)
        live.update("[bold][yellow](5)[/yellow][/bold] Converting to USM (.dat)... [green]Done[/green]")
    print("[yellow](6)[/yellow] Copying back the result... ", end="")
    shutil.copy(path.join(temp_folder, "target.usm"), destination)
    print("[green]OK[/green]")
    print("[yellow](7)[/yellow] Cleaning up... ", end="")
